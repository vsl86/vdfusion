...
// DependencyStatus represents the state of required CLI tools
...
// GetBinDir returns the absolute path to the app's local bin directory
...
// CheckDependencies checks if the required CLI tools are present
func CheckDependencies() DependencyStatus {
	...
	for i, tool := range tools {
		// Use utils.Resolve which checks AppData/bin AND system PATH
		resolved := utils.Resolve(tool)

		// If Resolve returns just the name, it likely failed to find an absolute path
		// but we should verify if that command is actually runnable or if it's just a fallback string.
		// However, utils.Resolve is designed to return the best possible path.
		// If it's an absolute path, it exists.
		...
		// Fallback: check if exec.LookPath finds it (redundant but safe)
...
// DownloadDependencies downloads static FFmpeg binaries for the current platform
func DownloadDependencies(ctx context.Context, progress func(string, float64)) error {
	...
	case "darwin":
		if runtime.GOARCH == "arm64" {
			...
			// Evermeet.cx releases them separately
			urls = []string{
...
func downloadFile(ctx context.Context, url, dest string, progress func(float64)) error {
	...
	// Some sites block generic Go User-Agent
