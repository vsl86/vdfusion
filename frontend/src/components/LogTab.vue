    ...
    <div class="log-tab-container">
        ...
        <div class="terminal-container" ref="terminalRef">
            <div v-if="logs.length === 0" class="empty-logs">
                Waiting for logs... (Backend output will appear here)
            </div>
...
import { EventsOn, SaveLogToFile } from '../api'

const logs = ref([])
const terminalRef = ref(null)
const autoScroll = ref(true)

// Persist logs in session storage (up to a limit)
const MAX_LOGS = 2000

const loadSessionLogs = () => {
    const saved = sessionStorage.getItem('vdf_system_logs')
    if (saved) {
        try {
...
const saveSessionLogs = () => {
    sessionStorage.setItem('vdf_system_logs', JSON.stringify(logs.value.slice(-MAX_LOGS)))
}

onMounted(() => {
    loadSessionLogs()

    EventsOn('system_log', (line) => {
...
const clearLogs = () => {
    logs.value = []
    sessionStorage.removeItem('vdf_system_logs')
}
