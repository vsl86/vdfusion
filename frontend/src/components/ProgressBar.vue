<template>
  <div v-if="phase" class="progress-card card">
    <div class="card-body">
      ...
      <div class="progress-stats">
        ...
        <span v-else>Scanned: {{ current.toLocaleString() }} / {{ total.toLocaleString() }}</span>
...
const isIndeterminate = computed(() => {
  return phase.value === 'discovery' || phase.value === 'comparing'
})
...
const eta = computed(() => {
  if (phase.value !== 'scanning' || current.value <= 0 || total.value <= 1 || durationSeconds.value <= 0) return ''
  const rate = current.value / durationSeconds.value // files per second
  if (rate <= 0) return ''
  const remaining = (total.value - current.value) / rate
